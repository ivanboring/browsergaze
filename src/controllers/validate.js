...
const validate = {
    ...
    loadRules: function() {
        let confdir = helper.getConfigDirecory('entities');
        let files = fs.readdirSync(confdir);
        for (let file of files) {
            let ruledata = yaml.load(fs.readFileSync(confdir + file))
            let rule = file.replace('.yml', '');
            this.entityRules[rule] = ruledata;
        }
    },
    validateEntity: function(values, entityType) {
        // Load permissions into memory.
        if (Object.keys(this.entityRules).length === 0) {
            this.loadRules();
        }
        
        const validationErrors = [];
        const rules = this.entityRules[entityType].fields;
